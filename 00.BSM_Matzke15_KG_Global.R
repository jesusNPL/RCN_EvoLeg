...
wd = "/home/oscar/Documents/Plants_Biomes/BSM_biomes/"
wd = "/home/julian/Dropbox/Proyectos/Plants_Biomes/BSM_biomes/"
wd = "/home/julian/Documentos/Plants_Biomes/BSM_biomes/"
setwd(wd)
...
# However, you can find the extdata directory like this:
 ...
 #  /home/julian/R/x86_64-pc-linux-gnu-library/4.0/BioGeoBEARS/extdata
...
# extdata_dir = np(slashslash(paste(extdata_dir, "/Plants_Biomes", sep="/")))
...
# (from Ree & Smith 2008)
# "trfn" = "tree file name"
trfn = np(paste(addslash(extdata_dir), "QianZhang2014_410spp_11areas.newick", sep="")) # plants
...
# (from Ree & Smith 2008)
geogfn = np(paste(addslash(extdata_dir), "plants_410spp_11areas.data", sep="")) # plants
...
# BioGeoBEARS_run_object$timesfn = np(slashslash(paste(extdata_dir2, "timeperiods_isl_main.txt", sep="/")))
# BioGeoBEARS_run_object$dispersal_multipliers_fn = np(slashslash(paste(extdata_dir2, "dispersal_isl_main.txt", sep="/")))
...
runslow = TRUE
resfn = "Plants_DEC+J_BIOMES_v2.Rdata"
if (runslow)
{
...
#######################################################
pdffn = paste0("Plants_", model_name, "_BIOMES_v2.pdf")
pdf(pdffn, width=8.5, height=38)

analysis_titletxt = paste0(model_name, " on Plants")
...
res2 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"),
            plotwhat="text", label.offset=0.8, tipcex=0.5, statecex=0.5, splitcex=0.01, titlecex=0.8,
            plotsplits=F, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"),
            plotwhat="pie", label.offset=0.8, tipcex=0.5, statecex=0.5, splitcex=0.01, titlecex=0.8,
            plotsplits=F, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
...
# Non stratified = NULL (stratified=stratified)

############################################
# Open a PDF
############################################
pdffn = paste0(model_name, "_single_stochastic_map_n1_v2.pdf")
pdf(file=pdffn, width=8.5, height=38)
...
plot_BioGeoBEARS_results(results_object=resmod, analysis_titletxt="Stochastic map", addl_params=list("j"),
                         label.offset=0.8, plotwhat="text", cornercoords_loc=scriptdir, root.edge=TRUE,
...
paint_stochastic_map_branches(res=resmod, master_table_cladogenetic_events=master_table_cladogenetic_events,
                              colors_list_for_states=colors_list_for_states, lwd=5, lty=par("lty"), root.edge=TRUE)

plot_BioGeoBEARS_results(results_object=resmod, analysis_titletxt="Stochastic map", addl_params=list("j"),
                         plotwhat="text", cornercoords_loc=scriptdir, root.edge=TRUE, colors_list_for_states=colors_list_for_states,
...
pdffn = paste0(model_name, "_", length(clado_events_tables), "BSMs_v2_100_v2.pdf")
pdf(file=pdffn, width=8.5, height=38)
...
for (i in 1:nummaps_goal)
{
  ...
  plot_BSM(results_object=res, clado_events_table=clado_events_table, stratified=stratified, analysis_titletxt=analysis_titletxt,
           addl_params=list("j"), label.offset=0.8, plotwhat="text", cornercoords_loc=scriptdir, root.edge=TRUE,
           colors_list_for_states=colors_list_for_states, show.tip.label=TRUE, include_null_range=include_null_range)
...
hist_event_counts(counts_list, pdffn=paste0(model_name, "_histograms_of_event_counts_100BSM_v2.pdf"))
...
save.image("BSM_Plants_BIOMES_v2.RData")
